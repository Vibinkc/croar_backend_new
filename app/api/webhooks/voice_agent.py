...
async def vapi_webhook(request: Request, background_tasks: BackgroundTasks):
    ...
    if message_type == "end-of-call-report":
        call_info = payload.get("message", {})
        call_id = call_info.get("call", {}).get("id")
        ...
        background_tasks.add_task(process_vapi_transcript, call_id, transcript, recording_url, background_tasks)

    return {"status": "success"}

async def process_vapi_transcript(call_id: str, transcript: str, recording_url: str, background_tasks: BackgroundTasks):
    """
    Background worker to find the associated application and process the transcript.
    """
    async with db_manager.management_session() as session:
        ...
        log.append({
            ...
            "extracted": intelligence.get("values_extracted", {})
        })
        
        # Update app score based on interview
        app.ai_match_score = intelligence.get("score", app.ai_match_score)
        
        # Set status back to allow agent to make next decision
        app.ai_feedback = {
            **(app.ai_feedback or {}), 
            "agent_log": log,
            "agent_status": "VOICE_INTERVIEW_COMPLETED"
        }
