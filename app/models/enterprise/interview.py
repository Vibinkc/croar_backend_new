...
from sqlalchemy import String, Integer, TIMESTAMP, func, ForeignKey, Text, Boolean, Numeric, Date, ARRAY
from sqlalchemy.dialects.postgresql import UUID, JSONB, ENUM
from sqlalchemy.orm import Mapped, mapped_column, relationship
from sqlalchemy.sql import text
from . import EnterpriseBase
from .candidate import CandidateApplication
...
class InterviewAutomation(EnterpriseBase):
    ...
    time_slots: Mapped[Optional[list[str]]] = mapped_column(ARRAY(String), nullable=True) # Array of HH:MM strings
    google_meet_link: Mapped[Optional[str]] = mapped_column(String, nullable=True)
    
    interview_type: Mapped[str] = mapped_column(String(50), default="GMEET", server_default="GMEET")
    interview_template_id: Mapped[Optional[str]] = mapped_column(UUID(as_uuid=True), ForeignKey("interviews.id", ondelete="SET NULL"), nullable=True)
    
    email_template_id: Mapped[Optional[str]] = mapped_column(UUID(as_uuid=True), ForeignKey("email_templates.id", ondelete="SET NULL"), nullable=True)
...
class InterviewSchedule(EnterpriseBase):
    ...
    interview_id: Mapped[Optional[str]] = mapped_column(UUID(as_uuid=True), ForeignKey("interviews.id", ondelete="CASCADE"), nullable=True)
    automation_id: Mapped[Optional[str]] = mapped_column(UUID(as_uuid=True), ForeignKey("interview_automations.id", ondelete="CASCADE"), nullable=True)
    application_id: Mapped[Optional[str]] = mapped_column(UUID(as_uuid=True), ForeignKey("candidate_applications.id", ondelete="CASCADE"), nullable=True)
    interviewer_id: Mapped[Optional[str]] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
...
class InterviewAttempt(EnterpriseBase):
    ...
    schedule_id: Mapped[Optional[str]] = mapped_column(UUID(as_uuid=True), ForeignKey("interview_schedules.id", ondelete="SET NULL"), nullable=True)
