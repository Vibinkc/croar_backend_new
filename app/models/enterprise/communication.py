...
class EmailLog(EnterpriseBase):
    ...
    candidate_id: Mapped[str | None] = mapped_column(UUID(as_uuid=True), nullable=True)
    application_id: Mapped[str | None] = mapped_column(UUID(as_uuid=True), nullable=True)
...
class MailAutomation(EnterpriseBase):
    ...
    send_at: Mapped[TIMESTAMP | None] = mapped_column(TIMESTAMP, nullable=True)
