...
from sqlalchemy import String, Integer, TIMESTAMP, func, ForeignKey, Text, Boolean, Numeric, Date, Enum as SqlEnum
...
class X360Question(EnterpriseBase):
    ...
    category: Mapped[QuestionCategory] = mapped_column(SqlEnum(QuestionCategory), default=QuestionCategory.PERFORMANCE)
...
class X360AssessmentTemplate(EnterpriseBase):
    ...
    questions = relationship("X360TemplateQuestion", back_populates="template", order_by="X360TemplateQuestion.order")

class X360TemplateQuestion(EnterpriseBase):
    __tablename__ = "x360_template_questions"

    template_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("x360_assessment_templates.id", ondelete="CASCADE"), primary_key=True)
    question_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("x360_questions.id", ondelete="CASCADE"), primary_key=True)
...
class X360AssessmentCycle(EnterpriseBase):
    ...
    template_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("x360_assessment_templates.id", ondelete="SET NULL"), nullable=True)
    company_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("companies.id", ondelete="CASCADE"), nullable=False)
...
class X360AssessmentAssignment(EnterpriseBase):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, server_default=text("uuid_generate_v4()"))
    cycle_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("x360_assessment_cycles.id", ondelete="CASCADE"), nullable=False)
    ratee_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("employees.id", ondelete="CASCADE"), nullable=False)
    rater_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("employees.id", ondelete="CASCADE"), nullable=False)
...
class X360AssessmentResponse(EnterpriseBase):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, server_default=text("uuid_generate_v4()"))
    assignment_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("x360_assessment_assignments.id", ondelete="CASCADE"), nullable=False)
    question_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("x360_questions.id", ondelete="CASCADE"), nullable=False)
    
    answer_value: Mapped[Optional[int]] = mapped_column(Integer, nullable=True) # for RATING (1-5)
...
class X360EmployeeRaterMap(EnterpriseBase):
    """Explicit mapping of who can rate whom for specific relations if not following reporting structure."""
    ...
    rater_employee_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("employees.id", ondelete="CASCADE"), nullable=False)
    relation: Mapped[RelationType] = mapped_column(SqlEnum(RelationType), nullable=False)
