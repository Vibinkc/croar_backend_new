...
from app.models.enterprise.x360 import (
    ...
    X360EmployeeRaterMap, QuestionType, RelationType, CycleStatus, AssignmentStatus
)
...
class X360Service:
    ...
    async def start_cycle(self, db: AsyncSession, cycle_id: uuid.UUID, ratee_ids: List[uuid.UUID]):
        ...
        for ratee_id in ratee_ids:
            ...
            if ratee.reporting_to_id:
                await self._create_assignment(db, cycle_id, ratee_id, ratee.reporting_to_id, RelationType.MANAGER)
            ...
            for rater_map in map_res.scalars().all():
                await self._create_assignment(db, cycle_id, ratee_id, rater_map.rater_employee_id, rater_map.relation)
    ...
    async def _create_assignment(self, db: AsyncSession, cycle_id: uuid.UUID, ratee_id: uuid.UUID, rater_id: uuid.UUID, relation: RelationType):
    ...
    async def notify_raters(self, db: AsyncSession, cycle_id: uuid.UUID):
        stmt = select(X360AssessmentAssignment).where(
            and_(X360AssessmentAssignment.cycle_id == cycle_id, X360AssessmentAssignment.status == AssignmentStatus.PENDING)
        ).options(selectinload(X360AssessmentAssignment.rater), selectinload(X360AssessmentAssignment.cycle))
    ...
    async def get_report(self, db: AsyncSession, employee_id: uuid.UUID, cycle_id: uuid.UUID):
        ...
        cycle_stmt = select(X360AssessmentCycle).where(X360AssessmentCycle.id == cycle_id).options(
            selectinload(X360AssessmentCycle.template).selectinload(X360AssessmentTemplate.questions).selectinload(X360TemplateQuestion.question)
        )
        cycle_res = await db.execute(cycle_stmt)
        cycle = cycle_res.scalar_one_or_none()
        if not cycle or not cycle.template: return None
        template = cycle.template

        # 2. Fetch all completed assignments for this ratee in this cycle
        assign_stmt = select(X360AssessmentAssignment).where(
            and_(
                X360AssessmentAssignment.cycle_id == cycle_id,
                X360AssessmentAssignment.ratee_id == employee_id,
                X360AssessmentAssignment.status == AssignmentStatus.COMPLETED
            )
        ).options(selectinload(X360AssessmentAssignment.responses).selectinload(X360AssessmentResponse.question))
        ...
        total_cnt = await db.execute(select(func.count(X360AssessmentAssignment.id)).where(
            and_(X360AssessmentAssignment.cycle_id == cycle_id, X360AssessmentAssignment.ratee_id == employee_id)
        ))
        ...
        for ass in assignments:
            ...
            for resp in ass.responses:
                ...
                if resp.question.type == QuestionType.RATING and resp.answer_value:
                    ...
                    if rel not in category_scores[cat]: category_scores[cat][rel] = []
