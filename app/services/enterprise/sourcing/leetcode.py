...
class LeetCodeProvider(BaseScraperProvider):
    def __init__(self):
        super().__init__(
            platform_name="leetcode", site_domain="leetcode.com", result_pattern="leetcode.com/u/"
        )
    ...
    def search(
        ...
        queries = [f"site:leetcode.com/u/ {role}", f"site:leetcode.com/u/ {role} {loc}"]
        ...
        if username and password:
            for q in queries:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        for res in results:
                            ...
                            for item in organic_results:
                                ...
                                if url and "leetcode.com/" in url:
                                    url = url.split("?")[0]
                                    if url in seen_urls:
                                        continue
                                    seen_urls.add(url)
                                    ...
                                    if "LeetCode" in name:
                                        name = name.replace("LeetCode", "").strip()
                                    if not name:
                    ...
                    print(f"DEBUG: Oxylabs LeetCode exception: {e}")
