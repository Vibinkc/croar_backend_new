...
from app.models.enterprise.onboarding import (
    Onboarding, OnboardingStatus, OnboardingDocument, 
    OnboardingActivity, OnboardingTemplate
)
...
async def _prepare_onboarding_email(
    ...
    if onboarding.application and onboarding.application.job_requirement_id:
        ...
        comp_stmt = select(Company).join(JobRequirement).where(JobRequirement.id == onboarding.application.job_requirement_id)
...
async def initiate_onboarding_process(
    ...
    check_stmt = select(Onboarding).where(Onboarding.application_id == application_id).limit(1)
    ...
    template = None
    if template_id:
        template_stmt = select(OnboardingTemplate).where(OnboardingTemplate.id == template_id)
    ...
    if candidate:
        ...
        subject, body = await _prepare_onboarding_email(session, onboarding, subject, body, title="Welcome Aboard!")
        ...
        await log_onboarding_activity(
            session, onboarding.id,
            "Onboarding link sent to candidate",
            "System"
        )
...
async def send_onboarding_resubmit_email(
    ...
    subject, body = await _prepare_onboarding_email(session, onboarding, subject, body, title="Correction Needed")
...
async def send_onboarding_welcome_email(
    ...
    subject, body = await _prepare_onboarding_email(session, onboarding, subject, body, title="Welcome to the Team!")
