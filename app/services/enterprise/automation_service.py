...
async def trigger_automations(
    application_id: UUID, stage_index: int, session: AsyncSession, background_tasks: Any = None
):
    """
    ...
    context = {
        "ai_match_score": float(application.ai_match_score)
    ...
    for auto in automations:
        ...
        logger.info(
            f"Automation {auto.id} evaluation: {'PASSED' if passed else 'FAILED'} | Criteria: '{auto.criteria}' | Score: {context.get('ai_match_score')}"
        )
        if passed:
            # Handle Scheduling
            ...
            if is_immediate:
                ...
                if send_at and send_at.tzinfo:
                    send_at = send_at.astimezone(UTC).replace(tzinfo=None)

                effective_send_at = send_at or datetime.utcnow()
                logger.info(
                    f"Scheduling automated email for {candidate.email} at {effective_send_at} via Celery task"
                )
                ...
                send_scheduled_email_task.apply_async(
                    args=[str(auto.id), str(application.id), str(candidate.id), str(job.id)], eta=eta
                )
            ...
            if is_immediate and getattr(auto, "auto_move", False):
                ...
                if application.current_stage < max_stage:
                    logger.info(
                        f"Auto-moving application {application.id} to next stage {application.current_stage + 1} (Immediate)"
                    )
    ...
    for auto in as_automations:
        passed = await evaluate_criteria(auto.criteria, context)
        logger.info(
            f"Assessment Automation {auto.id} evaluation: {'PASSED' if passed else 'FAILED'} | Criteria: '{auto.criteria}'"
        )
        if passed:
            is_immediate = getattr(auto, "is_immediate", True)
            send_at = getattr(auto, "send_at", None)
            ...
            if is_immediate:
                ...
                effective_send_at = send_at or datetime.utcnow()
                logger.info(
                    f"Scheduling assessment invitation for {candidate.email} at {effective_send_at} via Celery task"
                )
                ...
                send_scheduled_assessment_task.apply_async(
                    args=[str(auto.id), str(application.id), str(candidate.id)], eta=eta
                )
    ...
    for auto in int_automations:
        passed = await evaluate_criteria(auto.criteria, context)
        logger.info(
            f"Interview Automation {auto.id} evaluation: {'PASSED' if passed else 'FAILED'} | Criteria: '{auto.criteria}'"
        )
        if passed:
