...
from app.models.enterprise.communication import MailAutomation, EmailLog, EmailTemplate
...
def send_scheduled_email_task(
    ...
    """
    return async_to_sync(_send_scheduled_email_async)(
        automation_id, application_id, candidate_id, job_id
    )
...
async def _send_scheduled_email_async(
    ...
    async with db_manager.session() as session:
        ...
        if not all([automation, application, candidate, job]):
            logger.error(f"Missing data for task: auto={bool(automation)}, app={bool(application)}, cand={bool(candidate)}, job={bool(job)}")
            return False
