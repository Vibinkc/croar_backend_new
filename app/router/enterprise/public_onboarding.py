...
from app.models.enterprise.onboarding import Onboarding, OnboardingDocument, OnboardingStatus, OnboardingActivity
...
async def get_public_onboarding(
    ...
    stmt = select(Onboarding).options(
        ...
        selectinload(Onboarding.application).selectinload(CandidateApplication.job_requirement).selectinload(JobRequirement.company)
    ...
    if onboarding.application and onboarding.application.job_requirement and onboarding.application.job_requirement.company:
...
async def submit_onboarding_info(
    token: UUID,
    request: Request,
    session: DBSessionDep
):
...
async def upload_onboarding_document(
    ...
    stmt = select(OnboardingDocument).where(
        OnboardingDocument.id == doc_id,
        OnboardingDocument.onboarding_id == token
    )
    result = await session.execute(stmt)
