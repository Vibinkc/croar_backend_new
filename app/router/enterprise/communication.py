...
from app.schemas.enterprise.communication import (
    ...
    EmailSendRequest, EmailDraftRequest, TemplateGenerationRequest
)
...
async def get_communication_context(
    ...
    company = None
    if job_id:
        stmt = select(Company).join(JobRequirement, Company.id == JobRequirement.company_id).where(JobRequirement.id == job_id).limit(1)
    ...
    recruiter_name = current_agent.first_name + " " + (current_agent.last_name or "") if current_agent.first_name else "Recruiting Team"
    ...
    return {
        ...
        "recruiter_email": current_agent.email
    }
...
async def send_emails(
    ...
    if request.recipient_ids:
        stmt = select(Candidate).where(
            Candidate.id.in_(request.recipient_ids)
        )
    ...
    default_stmt = select(Company).limit(1)
    ...
    for email_addr, candidate in recipients:
        ...
        if candidate:
            stmt = select(CandidateApplication, JobRequirement, Company)\
        ...
        candidate_name = getattr(candidate, "full_name", candidate.email if candidate else email_addr) or "Candidate"
        ...
        success, error = await run_in_threadpool(send_smtp_email, email_addr, final_subject, final_body, company_name, company_logo)
...
async def generate_template(
    ...
    if not _settings.openai_api_key:
        ...
        fallback_body = (
            ...
            f"We are reaching out regarding {{{{job_title}}}} at {{{{company_name}}}}.\n\n"
            f"Best regards,\n{{{{recruiter_name}}}}"
        )
        return {
            "name": request.purpose,
            "subject": f"Regarding your application – {{{{job_title}}}}",
            "body": fallback_body,
