...
async def list_assessment_automations(
    ...
    stmt = (
        ...
        .where(AssessmentAutomation.company_id == current_user.company_id)
...
async def create_assessment_automation(
    ...
    db_auto = AssessmentAutomation(**data, company_id=current_user.company_id)
    db.add(db_auto)
    await db.commit()
    ...
    stmt = (
        select(AssessmentAutomation)
        .where(
            AssessmentAutomation.id == db_auto.id, AssessmentAutomation.company_id == current_user.company_id
        )
        .options(selectinload(AssessmentAutomation.email_template))
    )
    result = await db.execute(stmt)
...
async def generate_questions(
    ...
    stmt = select(AssessmentAutomation).where(
        AssessmentAutomation.id == automation_id, AssessmentAutomation.company_id == current_user.company_id
    )
    result = await db.execute(stmt)
    ...
    stmt = (
        select(AssessmentAutomation)
        .where(
            AssessmentAutomation.id == automation_id,
            AssessmentAutomation.company_id == current_user.company_id,
        )
        .options(selectinload(AssessmentAutomation.email_template))
    )
    result = await db.execute(stmt)
