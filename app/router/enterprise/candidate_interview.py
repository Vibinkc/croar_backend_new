...
from app.models.enterprise.interview import InterviewSchedule, InterviewAttempt
...
async def verify_interview_session(
    ...
    ).order_by(InterviewSchedule.created_at.desc()).limit(1)
    result = await db.execute(stmt)
    ...
    if email and candidate_email:
        if email.strip().lower() != candidate_email.strip().lower():
            raise HTTPException(status_code=403, detail="Email verification failed. Please use your registered email.")
    ...
    if not user_id:
        ...
        if system_user:
            ...
            raise HTTPException(status_code=500, detail="System configuration error: No valid user found for interview session.")
...
async def complete_interview(
    ...
    """
    stmt = select(InterviewAttempt).options(
        selectinload(InterviewAttempt.schedule)
