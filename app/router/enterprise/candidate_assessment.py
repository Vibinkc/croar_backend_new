...
from app.models.enterprise.assessment import AssessmentAutomation, AssessmentTemplate, AssessmentAttempt, AssessmentType
...
async def verify_candidate_and_start(
    req: AssessmentAttemptCreate,
    ...
    if job_requirement_id:
        ...
        if not application:
            raise HTTPException(status_code=403, detail="You have not applied for the job associated with this test")
    else:
        # Fallback to the most recent application if no job specified
        stmt = select(CandidateApplication).where(
            CandidateApplication.candidate_id == candidate.id
        ...
        if not application:
            raise HTTPException(status_code=403, detail="No application found for this candidate")
...
async def get_test_data(
    ...
    stmt = select(AssessmentAttempt).options(
        selectinload(AssessmentAttempt.application).selectinload(CandidateApplication.job_requirement).selectinload(JobRequirement.company)
    ...
    company = None
    if attempt.application and attempt.application.job_requirement and attempt.application.job_requirement.company:
...
async def submit_test(
    ...
    for q in questions:
        ...
        if q_type == "APTITUDE":
            ...
            if len(code) > 100: cod_score_accum += 100
            elif len(code) > 50: cod_score_accum += 80
            elif len(code) > 10: cod_score_accum += 40
            else: cod_score_accum += 0
    ...
    await db.commit()
    return {"score": overall_score, "status": "COMPLETED", "moved": should_move if 'should_move' in locals() else False}
