"""Exhaustive integration tests for Onboarding Templates (/api/v1/enterprise/onboarding/templates)."""

import uuid

from app.models.shared.constants import ModuleScope, PermissionAction

BASE = "/api/v1/enterprise/onboarding/templates"
READ = [(ModuleScope.onboarding, PermissionAction.read)]
CREATE = [(ModuleScope.onboarding, PermissionAction.create)]
DELETE = [(ModuleScope.onboarding, PermissionAction.delete)]


async def _create(client, name=None):
    return await client.post(BASE + "/", json={"name": name or f"Onb-{uuid.uuid4().hex[:6]}"})
...
class TestCreate:
    ...
    async def test_happy_path_201(self, client, seed_company, as_user, auth_user):
        ...
        r = await _create(client, "Standard Onboarding")
        assert r.status_code == 201, r.text
        assert r.json()["name"] == "Standard Onboarding"
...
class TestGet:
    ...
    async def test_happy_path(self, client, seed_company, as_user, auth_user):
        as_user(auth_user(seed_company.id, perms=CREATE + READ))
        created = (await _create(client, "Fetch Onb")).json()
        r = await client.get(f"{BASE}/{created['id']}")
        assert r.status_code == 200
        assert r.json()["name"] == "Fetch Onb"
